...
const Quanlykhambenh: React.FC = () => {
  ...
  const [searchText, setSearchText] = useState<string>('');
  const [statusFilter, setStatusFilter] = useState<string>('all');
  const [selectedDate, setSelectedDate] = useState<dayjs.Dayjs | null>(dayjs()); // Mặc định là ngày hiện tại
  const [isModalVisible, setIsModalVisible] = useState<boolean>(false);
  const [isKetLuanModalVisible, setIsKetLuanModalVisible] = useState<boolean>(false);
  ...
  const checkHasPrescription = async (appointmentId: number): Promise<boolean> => {
    try {
      ...
      const hasPrescription = Array.isArray(data) ? data.length > 0 : !!data;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const currentDate = selectedDate ? selectedDate.format('YYYY-MM-DD') : dayjs().format('YYYY-MM-DD');
  ...
  useEffect(() => {
    ...
    if (selectedDate) {
      ...
      filtered = filtered.filter((appt) =>
        dayjs(appt.created_at).format('YYYY-MM-DD') === dateStr
      );
    }
